...
def fmt_wti(result: dict) -> str:
    ...
    lines = [
        f"📊 <b>WTI — {t} vs S&P500</b>  ({result['total_bars']} hari)\n",

        f"<code>Threshold {t:<6} : {result['tkr_threshold']:.2f}%"
        f"  (ATR14={result['atr_pct']:.2f}%)</code>",

        f"🟢 <b>S&P500 Up</b>   : <b>{result['spy_up_total']}</b> hari",
        f"   ✅ {t} up   : <b>{result['spy_up_pct']:.1f}%</b>  ({result['spy_up_match']} hari)",
        f"   ❌ {t} down  : <b>{result['spy_up_miss_pct']:.1f}%</b>  ({spy_up_miss} hari)\n",

        f"🔴 <b>SPY Down</b> : <b>{result['spy_dn_total']}</b> hari",
        f"   ✅ {t} down : <b>{result['spy_dn_pct']:.1f}%</b>  ({result['spy_dn_match']} hari)",
        f"   ❌ {t} up  : <b>{result['spy_dn_miss_pct']:.1f}%</b>  ({spy_dn_miss} hari)\n",
