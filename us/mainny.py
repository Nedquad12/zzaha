...
FASTAPI_RELOAD_URL = "http://localhost:8000/internal/reload"

def _trigger_api_reload():
    """Ping FastAPI agar reload RAM cache dari disk."""
    try:
        import requests as _req
        r = _req.post(FASTAPI_RELOAD_URL, timeout=10)
        if r.ok:
            import logging
            logging.getLogger(__name__).info(f"FastAPI RAM cache reloaded: {r.json()}")
    except Exception as e:
        import logging
        logging.getLogger(__name__).warning(f"Gagal ping FastAPI reload: {e}")
        
async def cmd_reload(update, context):
    """/reload — reload RAM cache FastAPI dari disk (hanya allowed IDs)."""
    from telegram.constants import ParseMode
    if not is_allowed(update.effective_user.id):
        await update.message.reply_text("⛔ Anda tidak memiliki akses.")
        return

    await update.message.reply_text("⏳ Mereload RAM cache...", parse_mode=ParseMode.HTML)

    try:
        import requests as _req
        r = _req.post(FASTAPI_RELOAD_URL, timeout=15)
        if r.ok:
            data = r.json()
            await update.message.reply_text(
                f"✅ <b>RAM Cache Reloaded</b>\n"
                f"📦 Tickers loaded : <b>{data.get('tickers', '?')}</b>\n"
                f"🕐 Reloaded at    : <code>{data.get('reloaded_at', '?')[:19]}</code>",
                parse_mode=ParseMode.HTML,
            )
        else:
            await update.message.reply_text(f"❌ FastAPI error: {r.status_code}")
    except Exception as e:
        await update.message.reply_text(f"❌ Tidak bisa reach FastAPI: {e}")
