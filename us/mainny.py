...
def _reset_all_data() -> dict:
    """
    Hapus semua data lama sebelum /9 berjalan:
      - cache/       → file JSON OHLCV harian
      - us/500/      → file JSON score history (500 bar)
      - us/train/    → train.db SQLite ML
      - us/score_*.xlsx → hasil scoring hari-hari sebelumnya

    Return dict summary untuk ditampilkan ke user.
    """
    summary = {}
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(callback_chart_format, pattern=r"^chart_.*\|(?:png|html)$"))
