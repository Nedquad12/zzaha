...
from cache     import reset_cache, save as cache_save, load as cache_load
from scorer    import calculate_all_scores
from storage   import save_to_xlsx
from formatter import fmt_alert, fmt_detail, fmt_top_bottom, fmt_ip_table
...
async def process_all_stocks(bot: Bot, chat_id: int):
    ...
    for ticker, df in raw_data.items():
        ...
        logger.info(f"  {ticker} → total: {result['total']:+.2f}  tight: {ts:+d}")
    ...
    if args:
        ...
        try:
            ...
            rows = df_sorted.rename(columns={
                ...
                "IP Score": "ip_score", "VSA": "vsa", "RSI": "rsi",
