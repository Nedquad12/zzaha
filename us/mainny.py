...
async def cmd_chart(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not args:
        await update.message.reply_text(
            ...
            "Format output: <b>PNG</b> (gambar) atau <b>HTML</b> (interaktif TradingView-style)",
            parse_mode=ParseMode.HTML)
        return
    ...
    if args[0].lower() == "ts":
        if len(args) < 2:
            await update.message.reply_text("⚠️ Format: <code>/ch ts TICKER</code>", parse_mode=ParseMode.HTML)
            return
        ticker = args[1].upper()
