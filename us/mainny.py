...
async def process_all_stocks(bot: Bot, chat_id: int):
    ...
    for ticker, df in raw_data.items():
        ...
        logger.info(
            ...
            f"tight: {ts:+d}  vfa: {result['vfa']:+d}  wcc: {result['wcc']:+d}"
        )
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "/wcc             — Tabel Wick Candle Change semua saham\n"
...
async def cmd_wcc(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Urutkan: WCC tertinggi (UP) dulu, lalu terendah (DOWN) di bawah
...
def main():
    ...
    app.add_handler(CommandHandler("scor",    cmd_ip))
