...
async def cmd_pred(update, context):
    args = context.args
    if not args:
        await update.message.reply_text(
            "🔮 <b>Prediksi 3 Hari</b>\n\n"
            "Penggunaan: <code>/pred TICKER</code>\n\n"
            "Contoh: <code>/pred NVDA</code>\n\n"
            "Menghasilkan 2 prediksi:\n"
            "  📊 <b>Score Model</b>  — apakah harga naik/turun/netral dalam 3 hari\n"
            "  📡 <b>WTI Model</b>    — apakah saham ikut arah SPY atau divergen\n\n"
    ...
    await update.message.reply_text(
        f"⏳ <b>Menjalankan prediksi untuk {ticker}...</b>\n"
        f"Training 2 model XGBoost, mohon tunggu...",
        parse_mode="HTML",
    )

    try:
        msgs = run_prediction(ticker)
        for msg in msgs:
            await update.message.reply_text(msg, parse_mode="HTML")
            import asyncio; await asyncio.sleep(0.5)
    except Exception as e:
        await update.message.reply_text(
            f"❌ Error prediksi {ticker}: {e}",
            parse_mode="HTML",
        )
...
async def cmd_help(update, context):
    ...
    text = (
        ...
        "🔮 <b>Prediksi</b>\n"
        "<code>/pred TICKER</code>  — Prediksi 3 hari: arah harga + korelasi SPY\n\n"
