...
def fmt_alert(r: dict) -> str:
    ...
    return (
        ...
        f"VSA:{r['vsa']:+d}  FSA:{r.get('fsa', 0):+d}  RSI:{r['rsi']:+d}  MACD:{r['macd']:+d}  "
...
def fmt_ip_table(rows: list[dict]) -> list[str]:
    ...
    header = [
        ...
        "<pre>",
        f"{'Ticker':<7} {'Total':>6}  {'IP':>5}  {'VSA':>4}  {'FSA':>4}  {'RSI':>4}  {'MACD':>5}  {'MA':>4}  {'T':>3}",
        "─" * 58,
    ]
    ...
    for r in rows:
        ...
        data_lines.append(
            ...
            f"{r['vsa']:>+4d}  {r.get('fsa', 0):>+4d}  {r['rsi']:>+4d}  {r['macd']:>+5d}  {r['ma']:>+4d}  {t:>+3d}"
        )
