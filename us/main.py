...
from formatter import fmt_alert, fmt_detail, fmt_top_bottom, fmt_ip_table, fmt_vfa_table
from chart     import generate_chart
from tight     import scan_tight, score_tight, format_vt, format_t
from indicators import get_vfa_detail
...
async def process_all_stocks(bot: Bot, chat_id: int):
    ...
    for ticker, df in raw_data.items():
        ...
        logger.info(f"  {ticker} → total: {result['total']:+.2f}  tight: {ts:+d}  vfa: {result['vfa']:+d}")
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "🤖 <b>Stock Score Bot</b>\n\n"
        "<b>Restricted:</b>\n"
        "/9               — Hitung skor semua saham\n"
        "/ip              — Tabel skor hari ini\n"
        "/ip TICKER       — Detail skor satu saham\n\n"
        "<b>Public:</b>\n"
        "/vtus            — Daftar saham Very Tight\n"
        "/tus             — Daftar saham Tight\n"
        "/vfa             — Tabel Volume Freq Analysis semua saham\n"
        "/chart TICKER [method] [sens] — Chart S&R\n\n"
        "Method chart: donchian(d) | pivots(p) | csid(c) | zigzag(z)",
        parse_mode=ParseMode.HTML,
    )
...
async def cmd_ip(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if args:
        ...
        # Hitung tight score real-time untuk 1 ticker
        vt_list, t_list = scan_tight()
        vt_set = {s["ticker"] for s in vt_list}
        t_set  = {s["ticker"] for s in t_list}
        ts     = score_tight(ticker, vt_set, t_set)
        ...
        try:
            ...
            rows = df_sorted.rename(columns={
                ...
                "VFA": "vfa", "RSI": "rsi", "MACD": "macd", "MA": "ma",
                "Tight": "tight",
            }).to_dict("records")
...
async def cmd_vfa(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for ticker in tickers:
        ...
        rows.append({
            ...
            "total":    0.0,   # tidak ada total tanpa run /9
        })
    ...
    # Sort by VFA score tertinggi dulu, lalu by avg_freq descending
...
async def cmd_chart(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/chart TICKER [METHOD] [SENSITIVITY] — semua member bisa akses."""
    args = context.args
    if not args:
        await update.message.reply_text(
            "⚠️ Format: <code>/chart TICKER [method] [sensitivity]</code>\n"
            "Method: donchian(d) | pivots(p) | csid(c) | zigzag(z)\n"
            "Contoh: <code>/chart AAPL pivots 10</code>",
            parse_mode=ParseMode.HTML)
        return
