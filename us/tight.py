"""
tight.py — Gabungan tight_tracker + tight_full
Deteksi saham Very Tight (VT) dan Tight (T) berdasarkan jarak harga ke MA.

Definisi:
  VT (Very Tight) : close > MA3/5/10/20 DAN jarak ke semua MA < 5%
  T  (Tight)      : close > MA3/5/10/20 DAN jarak ke semua MA antara 5%-7%

Scoring:
  Masuk VT saja          → +1
  Masuk VT dan T         → +2  (logikanya: sangat rapat = lebih kuat)
  Tidak masuk VT/T       → -1
  Tidak masuk VT, masuk T → 0

Data diambil dari cache OHLCV yang sudah ada (hasil /9).
MA dihitung langsung dari data close, periode: 3, 5, 10, 20.
"""
...
T_MAX          = 7.0    # jarak maksimal untuk T (%)
...
def scan_tight() -> tuple[list[dict], list[dict]]:
    """
    Scan semua ticker di cache, pisahkan ke list VT dan T.

    Returns:
        (vt_list, t_list) — masing-masing diurutkan by value desc
    """
    tickers = list_cached()
    vt_list = []
    t_list  = []
...
def score_tight(ticker: str, vt_set: set, t_set: set) -> int:
    """
    Hitung tight score untuk satu ticker berdasarkan set VT dan T.

    Args:
        ticker : kode saham
        vt_set : set ticker yang masuk VT
        t_set  : set ticker yang masuk T

    Returns:
        +2 masuk keduanya
        +1 masuk VT saja
         0 masuk T saja
        -1 tidak masuk keduanya
    """
...
def _format_table(title: str, subtitle: str, results: list[dict]) -> str:
    if not results:
    ...
    msg += f"{title}\n"
    msg += f"{subtitle}\n\n"
    msg += f"{'Kode':<6} {'Close':>8} {'MA20':>8} {'Vol(M)':>8} {'Val(B)':>7}\n"
    msg += "-" * 44 + "\n"
    ...
    msg += "Vol=Juta lembar, Val=Miliar (close×vol)\n"
    msg += "```"
    return msg
...
def format_vt(vt_list: list[dict]) -> str:
    return _format_table(
        "Very Tight Stocks (US)",
        "Close > MA3/5/10/20, jarak < 5%, Val ≥ 0.5B",
        vt_list,
    )
...
def format_t(t_list: list[dict]) -> str:
    return _format_table(
        "Tight Stocks (US)",
        "Close > MA3/5/10/20, jarak 5–7%, Val ≥ 0.5B",
        t_list,
    )
